...
本文档面向业务后端联调使用，依据《浙江省调在线建模_最小化实现路径技术文档.docx》的最小闭环整理。当前算法服务采用 FastAPI + SQLAlchemy，默认数据库为 SQLite，并预留 PostgreSQL URL 配置。本阶段后端只需要围绕 `task_id` 串联数据接入、建模、评估、发布和推理。工程重构说明见 `docs/refactor_algorithm_engine.md`。
